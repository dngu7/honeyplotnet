...
def create_bar_chart(x_data, text, f_name, xhat_data=None, bar_width=0.75, step=None):

    if xhat_data is not None:
        _create_bar_chart_double(x_data=x_data, xhat_data=xhat_data, text=text, f_name=f_name, bar_width=bar_width, step=step)
    else:
        _create_bar_chart_single(x_data=x_data, text=text, f_name=f_name, base_width=bar_width, step=step)


def _create_bar_chart_single(x_data, text, f_name, base_width=0.25, flip_thres=4, step=None):
    ...
    if step is None:
        cidx = random.choice(range(len(COLOR_PALETTES)))
        cmap = COLOR_PALETTES[cidx]
    else:
        cmap = COLOR_PALETTES[step % len(COLOR_PALETTES)]
    ...
    for idx, (y, label)in enumerate(zip(recon_values, text['series_name'])):
        ...
        if max_cat > flip_thres:
            plt.barh(br1, y, height=bar_width, label=label, color=color)
        else:
            plt.bar(br1, y, width=bar_width, label=label, color=color)
...
def _create_bar_chart_double(x_data, xhat_data, text, f_name, bar_width=0.25, step=None):
...
def create_scatter(x_data, text, f_name, xhat_data=None, bar_width=0.25, step=None):

    if xhat_data is not None:
        _create_scatter_double(x_data=x_data, xhat_data=xhat_data, text=text, f_name=f_name, step=step)
    else:
        _create_scatter_single(x_data=x_data, text=text, f_name=f_name, step=step)

def _create_scatter_single(x_data, text, f_name, markersize=4, step=None):
    ...
    if step is None:
        cidx = random.choice(range(len(COLOR_PALETTES)))
        cmap = ListedColormap(COLOR_PALETTES[cidx])
    else:
        cmap = ListedColormap(COLOR_PALETTES[step % len(COLOR_PALETTES)])
    ...
    for idx, (x, y, label) in enumerate(zip(x_values, y_values, text['series_name'])):
        ...
        plt.scatter(x, y, cmap=cmap, label=label, s=markersize)
...
def _create_scatter_double(x_data, xhat_data, text, f_name, step=None):
    ...
    for idx, (x,y, label) in enumerate(zip(x_values, y_values, text['series_name'])):
        if 'unnamed' in label:
            label = None
        plt.scatter(x, y, cmap=cmap, label=label)
...
def create_boxplot(x_data, text, f_name, xhat_data=None, step=None):
...
def create_recon_plots(x, x_hat, text_data, step, epoch_dir):
    ...
    for idx, (x_data, xhat_data, text) in enumerate(zip(x_chart_data, xhat_chart_data, text_data)):
        ...
        if x_data['head_type'] == 'categorical':
            create_bar_chart(x_data=x_data, xhat_data=xhat_data, text=text, f_name=f_name, step=step)
        elif x_data['head_type'] == 'point':
            create_line(x_data=x_data, xhat_data=xhat_data, text=text, f_name=f_name, step=step)
        else:
            create_boxplot(x_data=x_data, xhat_data=xhat_data, text=text, f_name=f_name, step=step)


def create_line(x_data, text, f_name, xhat_data=None, bar_width=0.25, markersize=3, step=None):

    if xhat_data is not None:
        _create_line_double(x_data=x_data, xhat_data=xhat_data, text=text, f_name=f_name, markersize=markersize, step=step)
    else:
        _create_line_single(x_data=x_data, text=text, f_name=f_name, markersize=markersize, step=step)

def _create_line_single(x_data, text, f_name, markersize=4, linewidth=2, step=None):
...
def _create_line_double(x_data, xhat_data, text, f_name,  markersize=4, linewidth=2, step=None):
...
def create_single_plot(x, text_data, save_dir, step):
    ...
    for idx, x_data in enumerate(x_data_mpl):
      ...
      if x_data['head_type'] == 'categorical':
          create_bar_chart(x_data, text, f_name, step=step)
      elif x_data['head_type'] == 'point':
          create_line(x_data, text, f_name, step=step)
      elif x_data['head_type'] == 'boxplot':
          create_boxplot(x_data, text, f_name, step=step)
      else:
