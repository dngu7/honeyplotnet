...
class PmcContinuousDataset(object):
  def __init__(self, data, 
    ...
    assert norm_mode in ['minmax'] #Offset does not work!
    ...
    self.scale_eps   = scale_eps #scale_eps[0] if isinstance(scale_eps, list) else scale_eps
    self.scale_floor = scale_floor #scale_floor[0] if isinstance(scale_floor, list) else scale_floor
  ...
  def perform_checks(self, outputs):
      flag = False
      if len(outputs['chart_data']['norm_series']) == 0 or \
          len(outputs['chart_data']['norm_series'][0]['data']) == 0 or \
            len(outputs['chart_data']['norm_scale']) == 0:
