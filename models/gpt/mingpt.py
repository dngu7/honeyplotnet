...
Forked from: https://github.com/karpathy/minGPT/
GPT model:
...
class Block(nn.Module):
    ...
    def forward(self, x, attn_mask=None, layer_past=None, return_present=False):
        # TODO: check that training still works
        if return_present: assert not self.training
        # layer past: tuple of length two with B, nh, T, hs
        attn, present = self.attn(self.ln1(x), attn_mask=attn_mask, layer_past=layer_past)
...
class GPTNoEmbed(nn.Module):
    """  the full GPT language model, with a context size of block_size. removes embeddings and head. """
    def __init__(self, block_size, n_layer=12, n_head=8, n_embd=256,
                 embd_pdrop=0., resid_pdrop=0., attn_pdrop=0., n_unmasked=0, use_pos_embs=True):
        super().__init__()
    ...
    def prep_atten_mask(self, attn_mask):
        if attn_mask is not None:
            ...
            attn_mask = attn_mask.to(dtype=self.dtype)  # fp16 compatibility
            attn_mask = (1.0 - attn_mask) * -10000.0
        return attn_mask


    def forward_with_past(self, embeddings, past=None, past_length=None):
        # inference only
        assert not self.training
        ...
        if past is not None:
            ...
            past = torch.cat(past, dim=-2)   # n_layer, 2, b, nh, len_past, dim_head
            past_shape = list(past.shape)
            expected_shape = [self.config.n_layer, 2, embeddings.shape[0], self.config.n_head, past_length, self.config.n_embd//self.config.n_head]
            assert past_shape == expected_shape, f"{past_shape} =/= {expected_shape}"
            position_embeddings = self.pos_emb[:, past_length, :]  # each position maps to a (learnable) vector
        else:
            position_embeddings = self.pos_emb[:, :embeddings.shape[1], :]
