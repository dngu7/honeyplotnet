...
from .mhd_helper import (
    get_reduce_fn, get_dist_loss, MLP1d
)

class MHDropoutNetRandom1D(nn.Module):
    def __init__(self,         inp_dim, 
        ...
        **kwargs):
        ...
        if self.bottleneck: 
            self.proj = MLP1d(inp_dim, hidden_dim, hidden_dim, act='relu') 
            dec_inp_dim = hidden_dim
        else:
            dec_inp_dim = inp_dim

        self.decoder = MLP1d(dec_inp_dim, hidden_dim, out_dim, **decoder_cfg)
        
    def forward(self, x, y=None, wta_idx=None, wta_loss=None, hypothese_count=1, **kwargs):
        
        hypotheses = self.sample(x, hypothese_count=hypothese_count)
    ...
    def get_win_hypo(self, hypos, wta_idx):
    ...
    def create_mask(self, x):
        m_shape = x.shape[:2]
        m_prob = torch.ones(m_shape, dtype=self.dtype_float, device=x.device, requires_grad=x.requires_grad) * (1. - self.dropout_rate)
        m = torch.bernoulli(m_prob).unsqueeze(-1)
        return m

    def sample(self, x, hypothese_count):
        
        x = torch.transpose(x, -2, -1)
        
        #Infer the repeat structure
        x_shape = list(x.shape)
        bsz, seq_len = x_shape[0], x_shape[-1]
        ...
        #Reverse channels
        #x_repeat = torch.transpose(x_repeat, -2, -1)
        ...
        # Apply dropout here
        distd_hidden = hidden * self.create_mask(hidden)

        # [bs * hypo_count, out_dim, x, y]
    ...
    def get_dist_loss(self, hypos, y, topk=None):

        hypo_count = hypos.size(self.hypothese_dim)

        y_shape = [1] * len(hypos.shape)
        y_shape[self.hypothese_dim] = hypo_count

        #Create copies of y to match sample length
        y_expanded = y.unsqueeze(self.hypothese_dim).repeat(y_shape)
        
        dist_loss = self.dist_loss(hypos, y_expanded)
    ...
    def get_wta_loss(self, dist_loss, topk_idx):
        wta_idx = topk_idx[:,0]
